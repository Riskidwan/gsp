<!DOCTYPE html>
<html lang="id">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pelatihan Operator Molding Karet</title>
    <link rel="stylesheet" href="Website/css/services.css" />
</head>

<body>


  <!-- SLIDE 1: Hero Section -->
  <section class="hero" style="background: linear-gradient(135deg, #02a85a 0%, #02a85a 100%); color: #fff; min-height: 100vh; display: flex; align-items: center; padding: 0;">
    <div style="max-width:1200px; margin:auto; display:flex; flex-wrap:wrap; align-items:center; gap:32px; padding:56px 24px;">
      <div style="flex:1 1 420px; min-width:280px; z-index:2;">
        <h1 style="font-size:2.8rem; margin-bottom:0.7rem;">Molding Operator</h1>
        <p style="font-size:1.35rem; color:#bbdefb; margin-bottom:1.2rem;">Pelatihan Operator Mesin Injection Molding</p>
        <p style="font-size:1.13rem; color:#e3f2fd; margin-bottom:1.5rem; max-width:500px;">
            Program ini membekali peserta dengan keterampilan mengoperasikan mesin injection molding, mulai dari pemasangan mold hingga pengendalian kualitas produk cetakan sesuai standar industri.
        </p>
        <a href="https://wa.me/6281234567890" target="_blank" style="background:#42a5f5; color:#fff; padding: 0.8rem 2.2rem; font-weight:600; font-size:1.1rem; border-radius:32px; text-decoration:none;">Hubungi Kami Sekarang</a>
      </div>
      <div style="flex:1 1 420px; min-width:280px; text-align:center;">
        <img src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=900&h=500&fit=crop" alt="Cleaning Service" style="width:100%; max-width:440px; border-radius:18px; box-shadow:0 2px 16px rgba(0,0,0,0.13);" />
      </div>
    </div>
  </section>

  <!-- TENTANG PELATIHAN -->
  <section class="section" style="background:#f3f8ff;">
    <div class="container" style="display:flex; flex-wrap:wrap; gap:32px; align-items:center;">
      <div style="flex:1 1 340px; min-width:260px;">
        <h2 style="color:#1565c0;">Tentang Pelatihan Molding Operator</h2>
        <p style="font-size:1.08rem; color:#444;">
          Pelatihan ini mengajarkan pengoperasian mesin injection molding untuk memproduksi komponen karet. Peserta mempelajari pemasangan mold, pengaturan parameter mesin, dan pemeriksaan kualitas hasil cetakan.
        </p>
        <ul style="font-size:1rem; color:#444; margin-top:1rem; padding-left:18px;">
          <li>Pengoperasian mesin injection molding</li>
          <li>Pemasangan & pelepasan mold</li>
          <li>Pengaturan parameter mesin</li>
          <li>Pengendalian kualitas hasil cetakan</li>
        </ul>
      </div>
      <div style="flex:1 1 340px; min-width:260px; text-align:center;">
        <img src="https://images.unsplash.com/photo-1621707146454-c6b7151e0914?w=600&h=300&fit=crop" 
             alt="Mesin Injection Molding" 
             style="width:100%; max-width:340px; border-radius:14px;">
      </div>
    </div>
  </section>


    <section class="hero" style="background: #02a85a; color: #fff; min-height: 100vh; display: flex; align-items: center; padding: 0;">
        <div style="max-width:1200px; margin:auto; display:flex; flex-wrap:wrap; align-items:center; gap:32px; padding:56px 24px;">
            <div style="flex:1 1 420px; min-width:280px; z-index:2;">
                <h1 style="font-size:2.8rem; margin-bottom:0.7rem;">Operator Cetak Karet (Rubber Molding)</h1>
                <p style="font-size:1.35rem; color:#bbdefb; margin-bottom:1.2rem;">Pelatihan Operator Mesin Cetak Karet</p>
                <p style="font-size:1.13rem; color:#e3f2fd; margin-bottom:1.5rem; max-width:500px;">
                    Program ini membekali peserta dengan keterampilan mengoperasikan mesin cetak karet, mulai dari persiapan material kompon, pemasangan mold, hingga pengendalian kualitas produk karet sesuai standar industri.
                </p>
                <a href="https://wa.me/6282328085554" target="_blank" style="background:#42a5f5; color:#fff; padding: 0.8rem 2.2rem; font-weight:600; font-size:1.1rem; border-radius:32px; text-decoration:none;">Hubungi Kami Sekarang</a>
            </div>
            <div style="flex:1 1 420px; min-width:280px; text-align:center;">
                <img src="https://images.unsplash.com/photo-1621707146454-c6b7151e0914?w=900&h=500&fit=crop" alt="Mesin Cetak Karet" style="width:100%; max-width:440px; border-radius:18px; box-shadow:0 2px 16px rgba(0,0,0,0.13);" />
            </div>
        </div>
    </section>

    <section class="section" style="background:#f3f8ff;">
        <div class="container" style="display:flex; flex-wrap:wrap; gap:32px; align-items:center;">
            <div style="flex:1 1 340px; min-width:260px;">
                <h2 style="color:#1565c0;">Tentang Pelatihan Operator Cetak Karet</h2>
                <p style="font-size:1.08rem; color:#444;">
                    Pelatihan ini fokus pada pengoperasian mesin press cetak karet untuk memproduksi berbagai komponen karet presisi. Peserta akan mempelajari seluruh proses, mulai dari persiapan material kompon karet, pemasangan mold, pengaturan parameter mesin (suhu, tekanan, dan waktu), hingga pemeriksaan kualitas hasil cetakan.
                </p>
                <ul style="font-size:1rem; color:#444; margin-top:1rem; padding-left:18px;">
                    <li>Pengoperasian mesin press cetak karet</li>
                    <li>Pemasangan & pelepasan mold (cetakan)</li>
                    <li>Pengaturan parameter (suhu, tekanan, waktu)</li>
                    <li>Pengendalian kualitas produk karet</li>
                </ul>
            </div>
            <div style="flex:1 1 340px; min-width:260px; text-align:center;">
                <img src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&h=300&fit=crop"
                    alt="Produk Karet"
                    style="width:100%; max-width:340px; border-radius:14px;">
            </div>
        </div>
    </section>

  <div class="service-card">
          <img src="https://images.unsplash.com/photo-1616348436168-0b47c2ed1e20?w=400&h=180&fit=crop"
               alt="Proses Produksi" class="service-image">
          <h3>Proses Produksi</h3>
          <p>Pengoperasian mesin untuk memproduksi komponen karet sesuai desain.</p>

    <section class="section">
        <div class="container" style="display:flex; flex-wrap:wrap; gap:32px; align-items:center;">
            <div style="flex:1 1 340px; min-width:260px; text-align:center;">
                <img src="https://images.unsplash.com/photo-1527515637462-cff94eecc1ac?w=400&h=180&fit=crop"
                    alt="Keunggulan GSP"
                    style="width:100%; max-width:340px; border-radius:14px;">
            </div>
            <div style="flex:1 1 340px; min-width:260px;">
                <h2 style="color:#1565c0;">Mengapa Memilih GSP?</h2>
                <ul style="font-size:1rem; color:#444; margin-top:1rem; padding-left:18px;">
                    <li>Instruktur berpengalaman dari industri manufaktur karet</li>
                    <li>Kurikulum berbasis industri dan kebutuhan mitra</li>
                    <li>Fasilitas lengkap sesuai standar pabrik</li>
                    <li>Sertifikat resmi diakui industri</li>
                    <li>Penyaluran kerja ke perusahaan mitra</li>
                </ul>
            </div>
>>>>>>> 50f41c29f91ecf55192946faeb9bb743062d208d
        </div>
    </section>

    <section id="projects" class="section">
        <div class="container">
            <h2 class="section-title">Kegiatan & Hasil Pelatihan Cetak Karet</h2>
            <p class="section-subtitle">
                Dokumentasi kegiatan pelatihan dan hasil praktik pengoperasian mesin cetak karet oleh peserta LPK kami.
            </p>
            <div class="services-grid">
                <div class="service-card">
                    <img src="https://images.unsplash.com/photo-1581090700029-2c4f1f4fc3f2?w=400&h=180&fit=crop"
                        alt="Setting Mesin" class="service-image">
                    <h3>Setting Mesin Cetak Karet</h3>
                    <p>Peserta mempelajari cara mengatur parameter suhu, tekanan, dan waktu curing pada mesin press sesuai spesifikasi kompon karet.</p>
                </div>
                <div class="service-card">
                    <img src="https://images.unsplash.com/photo-1616348436168-0b47c2ed1e20?w=400&h=180&fit=crop"
                        alt="Pemasangan Mold" class="service-image">
                    <h3>Pemasangan Mold</h3>
                    <p>Latihan memasang dan melepas cetakan (mold) dengan prosedur yang aman dan efisien.</p>
                </div>
                <div class="service-card">
                    <img src="https://images.unsplash.com/photo-1605902711622-cfb43c44367f?w=400&h=180&fit=crop"
                        alt="Proses Produksi" class="service-image">
                    <h3>Proses Cetak (Molding)</h3>
                    <p>Pengoperasian mesin untuk memproduksi komponen karet seperti seal, o-ring, atau gasket sesuai standar.</p>
                </div>
                <div class="service-card">
                    <img src="https://images.unsplash.com/photo-1581090700227-4c4f50f1e3b0?w=400&h=180&fit=crop"
                        alt="Pemeriksaan Kualitas" class="service-image">
                    <h3>Pemeriksaan Kualitas</h3>
                    <p>Pemeriksaan visual dan dimensi hasil cetakan karet untuk memastikan tidak ada cacat produk (defect).</p>
                </div>
                <div class="service-card">
                    <img src="https://images.unsplash.com/photo-1581093588401-4b2a61ecfe5f?w=400&h=180&fit=crop"
                        alt="Pemeliharaan Mold" class="service-image">
                    <h3>Pemeliharaan Mold</h3>
                    <p>Praktik pembersihan dan perawatan rutin pada mold untuk menjaga kualitas hasil produksi.</p>
                </div>
                <div class="service-card">
                    <img src="https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=400&h=180&fit=crop"
                        alt="Kerja Tim" class="service-image">
                    <h3>Kerja Tim Produksi</h3>
                    <p>Peserta berkolaborasi dalam tim untuk mencapai target produksi harian secara efektif dan efisien.</p>
                </div>
            </div>
        </div>
    </section>

    <section class="section" style="background:#f8fbff;">
        <div class="container">
            <h2 style="color:#1565c0; text-align:center; font-size:2.5rem; margin-bottom:1rem;">Klien yang Pernah Dilayani</h2>
            <p style="text-align:center; font-size:1.1rem; color:#666; margin-bottom:3rem;">Dipercaya oleh perusahaan dan institusi ternama</p>
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(150px, 1fr)); gap:30px; align-items:center;"></div>
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(150px, 1fr)); gap:30px; align-items:center;">
                <div style="text-align:center;">
                    <img src="Website/images/pt Inkom.png" alt="Aneka Bonecom" style="max-width:100px;">
                    <p style="margin-top:10px; font-size:0.95rem; color:#333;">PT Industri Komkar Indonesia</p>
                </div>
                <div style="text-align:center;">
                    <img src="Website/images/pt casuarina.jpeg" alt="Casuarina" style="max-width:100px;">
                    <p style="margin-top:10px; font-size:0.95rem; color:#333;">PT Casuarina HarnessIndo</p>
                </div>
                <div style="text-align:center;">
                    <img src="Website/images/PT Mega Putra Garmen.png" alt="vivere" style="max-width:100px;">
                    <p style="margin-top:10px; font-size:0.95rem; color:#333;">PT Mega Putra Garmen</p>
                </div>
                <div style="text-align:center;">
                    <img src="Website/images/pt wisnu.png" alt="karet" style="max-width:100px;">
                    <p style="margin-top:10px; font-size:0.95rem; color:#333;">PT Wisnu Cakra Perdana</p>
                </div>
                <div style="text-align:center;">
                    <img src="Website/images/PT Sakura Seal Manufaktur Indonesia.png" alt="karet" style="max-width:100px;">
                    <p style="margin-top:10px; font-size:0.95rem; color:#333;">PT Sakura Seal Manufaktur Indonesia</p>
                </div>
            </div>
        </div>
    </section>

    <section class="section" style="background:#fff; padding:60px 20px;">
        <div class="container" style="max-width:600px; margin:auto;">
            <h2 style="color:#1976d2; text-align:center; font-size:2.5rem; font-weight:700; margin-bottom:10px;">
                Berikan Masukan LPK Kami
            </h2>
            <p style="text-align:center; font-size:1.1rem; color:#555; margin-bottom:30px;">
                Kami mengharapkan ulasan dari Anda tentang kualitas Pelatihan Operator Cetak Karet.
            </p>
            <form action="https://script.google.com/macros/s/AKfycbwJrh-8PkI90ta7PoKs_TyTQxa3jvAZCQrVjS8FHC88NVzz-2nYyw1dvRAWNIAX3aByxw/exec" method="POST" style="background:#f9fff9; padding:30px; border-radius:20px; box-shadow:0 10px 25px rgba(0,0,0,0.06);">
                <div class="form-group" style="margin-bottom:20px;">
                    <label for="nama" style="display:block; font-weight:500; margin-bottom:8px;">Nama Anda</label>
                    <input type="text" id="nama" name="nama" placeholder="Boleh dikosongkan (anonim)" style="width:100%; padding:12px; border:1px solid #ccc; border-radius:10px; font-size:1rem;">
                </div>
                <div class="form-group" style="margin-bottom:20px;">
                    <label for="layanan" style="display:block; font-weight:500; margin-bottom:8px;">Jenis Layanan</label>
                    <select id="layanan" name="layanan" style="width:100%; padding:12px; border:1px solid #ccc; border-radius:10px; font-size:1rem;">
                        <option value="">Pilih jenis layanan</option>
                        <option value="molding">Operator Cetak Karet (Molding)</option>
                        <option value="rubber">Rubber Seal</option>
                        <option value="wiring">Wiring Harness</option>
                        <option value="sewing">Sewing</option>
                        <option value="packing">Packing</option>
                    </select>
                </div>
                <div class="form-group" style="margin-bottom:20px;">
                    <label for="pesan" style="display:block; font-weight:500; margin-bottom:8px;">Pesan</label>
                    <textarea id="pesan" name="pesan" required placeholder="Tuliskan pengalaman Anda dengan layanan LPK kami, saran, atau kritik membangun..." style="width:100%; padding:12px; border:1px solid #ccc; border-radius:10px; font-size:1rem; min-height:130px;"></textarea>
                </div>
                <div style="text-align:center; margin-top:30px;">
                    <button type="submit" style="background-color:#1976d2; color:#fff; padding:12px 30px; font-size:1rem; font-weight:600; border:none; border-radius:8px; cursor:pointer; transition:background 0.3s;">
                        Kirim
                    </button>
                </div>
                <p style="text-align:center; font-size:0.9rem; color:#777; margin-top:20px;">
                    Terima kasih atas waktu dan kepercayaan Anda pada LPK kami.
                </p>
            </form>
        </div>
    </section>

    <section class="section" style="background:#f3f8ff; padding:100px 0;">
        <div class="container" style="max-width:1200px; margin:auto; padding:0 24px;">
            <h2 style="color:#1565c0; text-align:center; font-size:2.8rem; margin-bottom:3rem;">
                Testimoni Peserta Pelatihan Cetak Karet
            </h2>
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(350px, 1fr)); gap:40px;">
                <div style="background:#ffffff; border-radius:20px; padding:40px; box-shadow:0 10px 25px rgba(0,0,0,0.08);">
                    <div style="display:flex; align-items:center; margin-bottom:1.5rem;">
                        <img src="https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=80&h=80&fit=crop&crop=face" style="width:60px; height:60px; border-radius:50%; margin-right:15px;">
                        <div>
                            <h4 style="color:#1565c0; margin:0;">Rizky Pratama</h4>
                            <p style="color:#666; margin:0; font-size:0.9rem;">Magang di PT XYZ Rubber Parts</p>
                        </div>
                    </div>
                    <p style="font-size:1.1rem; color:#444; font-style:italic;">
                        "Pelatihan ini sangat membantu saya memahami proses cetak karet, mulai dari persiapan kompon, setting mesin press, hingga inspeksi produk jadi."
                    </p>
                    <div style="color:#ffd700; font-size:1.2rem; margin-top:1rem;">⭐⭐⭐⭐⭐</div>
                </div>
                <div style="background:#ffffff; border-radius:20px; padding:40px; box-shadow:0 10px 25px rgba(0,0,0,0.08);">
                    <div style="display:flex; align-items:center; margin-bottom:1.5rem;">
                        <img src="https://images.unsplash.com/photo-1581092334574-6c4f4f65a722?w=80&h=80&fit=crop&crop=face" style="width:60px; height:60px; border-radius:50%; margin-right:15px;">
                        <div>
                            <h4 style="color:#1565c0; margin:0;">Putri Amelia</h4>
                            <p style="color:#666; margin:0; font-size:0.9rem;">Magang di PT ABC Rubber Seal</p>
                        </div>
                    </div>
                    <p style="font-size:1.1rem; color:#444; font-style:italic;">
                        "Instruktur sangat berpengalaman. Saya jadi paham alur kerja di industri karet dan pentingnya menjaga kualitas di setiap batch produksi."
                    </p>
                    <div style="color:#ffd700; font-size:1.2rem; margin-top:1rem;">⭐⭐⭐⭐⭐</div>
                </div>
                <div style="background:#ffffff; border-radius:20px; padding:40px; box-shadow:0 10px 25px rgba(0,0,0,0.08);">
                    <div style="display:flex; align-items:center; margin-bottom:1.5rem;">
                        <img src="https://images.unsplash.com/photo-1544723795-3fb6469f5b39?w=80&h=80&fit=crop&crop=face" style="width:60px; height:60px; border-radius:50%; margin-right:15px;">
                        <div>
                            <h4 style="color:#1565c0; margin:0;">Fajar Nugraha</h4>
                            <p style="color:#666; margin:0; font-size:0.9rem;">Magang di PT Maju Jaya Rubber</p>
                        </div>
                    </div>
                    <p style="font-size:1.1rem; color:#444; font-style:italic;">
                        "Saya belajar banyak tentang setting mold, cara menentukan suhu dan tekanan yang tepat untuk berbagai jenis kompon, serta perawatan mesin."
                    </p>
                    <div style="color:#ffd700; font-size:1.2rem; margin-top:1rem;">⭐⭐⭐⭐⭐</div>
                </div>
            </div>
        </div>
    </section>

    <section class="section" style="background:#1976d2; color:#fff; text-align:center; padding:80px 0;">
        <div class="container">
            <h2 style="font-size:2.5rem; margin-bottom:1rem; color:#fff;">Siap Menjadi Operator Handal?</h2>
            <p style="font-size:1.2rem; margin-bottom:2rem; color:#bbdefb;">Hubungi kami sekarang untuk info pendaftaran!</p>
            <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:20px;">
                <a href="https://wa.me/6282328085554" target="_blank" style="background:#42a5f5; color:#fff; padding:14px 32px; border-radius:32px; text-decoration:none; font-weight:600; font-size:1.1rem;">
                    💬 WhatsApp Pendaftaran
                </a>
                <a href="tel:+6282328085554" style="background:transparent; color:#fff; padding:14px 32px; border:2px solid #42a5f5; border-radius:32px; text-decoration:none; font-weight:600; font-size:1.1rem;">
                    📞 Hubungi Sekarang
                </a>
            </div>
        </div>
    </section>

    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const form = document.querySelector("form");
            const btn = form.querySelector("button[type='submit']");
            const btnText = btn.textContent;

            form.addEventListener("submit", function(e) {
                e.preventDefault();

                btn.disabled = true;
                btn.style.opacity = "0.6";
                btn.textContent = "Mengirim...";


        <div class="form-group" style="margin-bottom:20px;">
          <label for="layanan" style="display:block; font-weight:500; margin-bottom:8px;">Jenis Layanan</label>
          <select id="layanan" name="layanan"
            style="width:100%; padding:12px; border:1px solid #ccc; border-radius:10px; font-size:1rem;">
            <option value="">Pilih jenis layanan</option>
            <option value="rubber">Rubber Seal</option>
            <option value="wiring">Wiring Harness</option>
            <option value="sewing">Sewing</option>
            <option value="packing">Packing</option>
            <option value="molding">Molding Operator</option>
          </select>
        </div>

        <div class="form-group" style="margin-bottom:20px;">
          <label for="pesan" style="display:block; font-weight:500; margin-bottom:8px;">Pesan</label>
          <textarea id="pesan" name="pesan" required
            placeholder="Tuliskan pengalaman Anda dengan layanan LPK kami, saran, atau kritik membangun..."
            style="width:100%; padding:12px; border:1px solid #ccc; border-radius:10px; font-size:1rem; min-height:130px;"></textarea>
        </div>

        <div style="text-align:center; margin-top:30px;">
          <button type="submit"
            style="background-color:#1976d2; color:#fff; padding:12px 30px; font-size:1rem; font-weight:600; border:none; border-radius:8px; cursor:pointer; transition:background 0.3s;">
            Kirim
          </button>
        </div>

        <p style="text-align:center; font-size:0.9rem; color:#777; margin-top:20px;">
          Terima kasih atas waktu dan kepercayaan Anda pada LPK kami.
        </p>
      </form>
    </div>
  </section>

  <!-- SLIDE: Testimoni Pemagang Molding Operator -->
<section class="section" style="background:#f3f8ff; padding:100px 0;">
  <div class="container" style="max-width:1200px; margin:auto; padding:0 24px;">
    <h2 style="color:#1565c0; text-align:center; font-size:2.8rem; margin-bottom:3rem;">
      Testimoni Pemagang Molding Operator
    </h2>
    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(350px, 1fr)); gap:40px;">

      <!-- Testimoni 1 -->
      <div style="background:#ffffff; border-radius:20px; padding:40px; box-shadow:0 10px 25px rgba(0,0,0,0.08);">
        <div style="display:flex; align-items:center; margin-bottom:1.5rem;">
          <img src="https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=80&h=80&fit=crop&crop=face" style="width:60px; height:60px; border-radius:50%; margin-right:15px;">
          <div>
            <h4 style="color:#1565c0; margin:0;">Rizky Pratama</h4>
            <p style="color:#666; margin:0; font-size:0.9rem;">Magang di PT XYZ Molding Industries</p>
          </div>
        </div>
        <p style="font-size:1.1rem; color:#444; font-style:italic;">
          "Magang ini membuat saya paham proses pengoperasian mesin molding mulai dari persiapan cetakan, setting parameter, hingga pengeluaran produk."
        </p>
        <div style="color:#ffd700; font-size:1.2rem; margin-top:1rem;">⭐⭐⭐⭐⭐</div>
      </div>

      <!-- Testimoni 2 -->
      <div style="background:#ffffff; border-radius:20px; padding:40px; box-shadow:0 10px 25px rgba(0,0,0,0.08);">
        <div style="display:flex; align-items:center; margin-bottom:1.5rem;">
          <img src="https://images.unsplash.com/photo-1581092334574-6c4f4f65a722?w=80&h=80&fit=crop&crop=face" style="width:60px; height:60px; border-radius:50%; margin-right:15px;">
          <div>
            <h4 style="color:#1565c0; margin:0;">Putri Amelia</h4>
            <p style="color:#666; margin:0; font-size:0.9rem;">Magang di PT ABC Plastic Molding</p>
          </div>
        </div>
        <p style="font-size:1.1rem; color:#444; font-style:italic;">
          "Instruktur membimbing saya memahami alur kerja mesin molding dan pentingnya kontrol kualitas setiap batch produksi."
        </p>
        <div style="color:#ffd700; font-size:1.2rem; margin-top:1rem;">⭐⭐⭐⭐⭐</div>
      </div>

      <!-- Testimoni 3 -->
      <div style="background:#ffffff; border-radius:20px; padding:40px; box-shadow:0 10px 25px rgba(0,0,0,0.08);">
        <div style="display:flex; align-items:center; margin-bottom:1.5rem;">
          <img src="https://images.unsplash.com/photo-1544723795-3fb6469f5b39?w=80&h=80&fit=crop&crop=face" style="width:60px; height:60px; border-radius:50%; margin-right:15px;">
          <div>
            <h4 style="color:#1565c0; margin:0;">Fajar Nugraha</h4>
            <p style="color:#666; margin:0; font-size:0.9rem;">Magang di PT Maju Jaya Molding</p>
          </div>
        </div>
        <p style="font-size:1.1rem; color:#444; font-style:italic;">
          "Saya belajar setting mold, pengaturan suhu dan tekanan, serta prosedur perawatan mesin agar tetap optimal."
        </p>
        <div style="color:#ffd700; font-size:1.2rem; margin-top:1rem;">⭐⭐⭐⭐⭐</div>
      </div>

    </div>
  </div>
</section>

<!-- CTA Section -->
  <section class="section" style="background:linear-gradient(135deg, #02a85a 0%, #02a85a 100%); color:#fff; text-align:center; padding:80px 0;">
    <div class="container">
      <h2 style="font-size:2.5rem; margin-bottom:1rem; color:#fff;">Butuh Info Pemagangan?</h2>
      <p style="font-size:1.2rem; margin-bottom:2rem; color:#bbdefb;">Hubungi kami sekarang!</p>
      <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:20px;">
        <a href="https://wa.me/6281234567890" target="_blank" style="background:#42a5f5; color:#fff; padding:14px 32px; border-radius:32px; text-decoration:none; font-weight:600; font-size:1.1rem;">
          🧹 WhatsApp Pendaftaran
        </a>
        <a href="tel:+6281234567890" style="background:transparent; color:#fff; padding:14px 32px; border:2px solid #42a5f5; border-radius:32px; text-decoration:none; font-weight:600; font-size:1.1rem;">
          📞 Call Now
        </a>
      </div>
    </div>
  </section>


  <script>
    document.addEventListener("DOMContentLoaded", function() {
      const form = document.querySelector("form");
      const btn = form.querySelector("button[type='submit']");
      const btnText = btn.textContent;

      form.addEventListener("submit", function(e) {
        e.preventDefault();

        btn.disabled = true;
        btn.style.opacity = "0.6";
        btn.textContent = "Mengirim...";

        fetch(form.action, {
            method: "POST",
            body: new FormData(form),
          })
          .then((res) => res.text())
          .then((data) => {
            alert("✅ Masukan berhasil dikirim!");
            form.reset();

            btn.disabled = false;
            btn.style.opacity = "1";
            btn.textContent = btnText;
          })
          .catch((err) => {
            alert("❌ Gagal mengirim. Coba lagi.");
            console.error(err);

            btn.disabled = false;
            btn.style.opacity = "1";
            btn.textContent = btnText;
          });
      });
    });
  </script>

    <script src="Website/js/scripts.js"></script>

    <?= $this->endSection() ?>

</body>

</html>